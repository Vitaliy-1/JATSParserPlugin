...
<form class="pkp_form" id="ArticleGalleyForm" method="post" action="{url op="updateGalley" submissionId=$submissionId representationId=$representationId}">
	...
	{* Show check box to pick this galley for displaying on article landing page *}
	{if $articleGalleyFile && array_search($articleGalleyFile->getFileType(), array("application/xml", "text/xml"))}
		{*
		{fbvFormSection for="jatsParserDisplayXml" list=true}
			{fbvElement type="checkbox" label="plugin.generic.jastParser.displayXml" value=$jatsParserDisplayXml id="jatsParserDisplayXml"}
		{/fbvFormSection}
		*}
		{capture assign=jatsParserDisplayXmlUrl}{url component="plugins.generic.jatsParser.JatsParserHandler" op="fetchGrid"}{/capture}
		{load_url_in_div id="jatsParserDisplayXml" url=$jatsParserDisplayXmlUrl}
	{/if}
